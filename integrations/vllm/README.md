...
`kapsl-vllm-connector-v0.7.0`. The established tag namespace is retained
across the repository migration so existing release automation and provenance
remain unambiguous.
...
publisher for repository `kapsl-runtime/kapsl-integrations`, workflow
`publish-vllm-connector.yml`, and GitHub environment `pypi-vllm-connector`.
...
backend-native CUDA attention probes in the
[SDK conformance guide](https://github.com/kapsl-runtime/kapsl-sdk/blob/main/docs/backend-kv-conformance.md);
